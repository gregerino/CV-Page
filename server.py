#!/usr/bin/env python3
"""
Marcus Hultberg CV — local server with Claude API proxy.
Set your API key: export ANTHROPIC_API_KEY=sk-ant-...
Then run: python3 server.py
"""
import http.server, json, os, urllib.request, socketserver, posixpath, urllib.parse
...
class CVHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not api_key:
            self._json(400, {"error": "ANTHROPIC_API_KEY not set. Run: export ANTHROPIC_API_KEY=sk-ant-..."})
            return
        if not question:
...
if __name__ == "__main__":
    with socketserver.TCPServer(("", PORT), CVHandler) as httpd:
        ...
        print("Make sure ANTHROPIC_API_KEY is set for the AI search.")
        httpd.serve_forever()
